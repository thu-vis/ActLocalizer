...
parser.add_argument('--mode', default='active',
                    choices=['single', 'weakly', 'fully', "active"], help='traininig mode')
parser.add_argument('--active-ratio', type=float, default=0.1,
                    help='active sampling ratio')
parser.add_argument('--log-dir', default='./logs',
                    help='diretory for saving logs')
parser.add_argument('--model-dir', default='./ckpt',
                    help='diretory for saving models')
parser.add_argument('--feature-path', default=os.path.join(os.path.dirname(__file__),'./data'), help='feature path')
parser.add_argument('--buffer-path', default=os.path.join(os.path.dirname(__file__),'./buffer'), help='buffer path')
...
parser.add_argument('--dataset-name', default='Thumos19',
                    help='dataset to train on (default: Thumos19)')
...
# test model on train/validation
parser.add_argument('--test-on-train', action='store_true', default=False,
                    help='test on train, default false')
# save output in result/outputs
...
parser.add_argument('--prop', action='store_true',
                    default=False, help='propagation')
parser.add_argument('--export', action='store_true',
                    default=False, help='export')
parser.add_argument('--increase', action='store_true',
                    default=False, help='increase 1 when get rightbound')
parser.add_argument('--A', type=int,
                    default=1, help='A hyper-parameter')