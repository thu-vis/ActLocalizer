...
def evaluate(itr,
    ...
    for idx in inds:
        ...
        pred_frames = tcam.argmax(axis=1)
        T_num, F_num = get_true_and_false(pred_frames, frame_label)
        all_F += F_num
        all_T += T_num
        if background:
            tcam = tcam[:, 1:]
        ...
        segments, frames = generate_segment(
            tcam, score, threshold_type=threshold_type, 
            frame_type=frame_type, act_weight=act_weight, 
    ...
    prediction_filename = os.path.join(prediction_output, 'prediction.json')
    ...
    for i in range(len(dmap)):
        tb_writer.log_value('mAP/IoU@%s' % (str(tiou_thresholds[i])), dmap[i],
                         itr)
    tb_writer.log_value('mAP/Average IoU', dmap.mean(),
                        itr)
