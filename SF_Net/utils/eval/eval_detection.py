...
from ..utils import get_logger, update_default_level
...
class ANETdetection(object):
    ...
    def __init__(self, ground_truth_filename=None, predict_data=None,
        ...
        self.prediction = self._import_prediction(predict_data)
        logger = get_logger()
        if self.verbose:
    ...
    def evaluate(self):
        ...
        self.pAP = compute_average_precision_detection(self.ground_truth, self.prediction, self.tiou_thresholds)
        #  self.cap = self.compute_porposal_average_precision()

        if self.verbose:
            logger.info('\tmAP: {}'.format(','.join(map(lambda x: str(round(x,5)),self.mAP))))
            logger.info('\tAverage-mAP: {}'.format(self.average_mAP))
            logger.info('\tpropsoal AP: {}'.format(','.join(map(lambda x: str(round(x,5)), self.pAP))))
            # logger.info('\t\teval_detection: ' + str(self.   average_mAP))
        return self.mAP
